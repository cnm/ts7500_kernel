/*
 * linux/drivers/usb/gadget/str8100_udc.c
 * Star Semi. on-chip full and high speed USB device controllers
 *
 * Copyright (C) 2004-2008 Star Semiconductor Corporation - Scott Shu
 *
 ...
 *  - EP maxpacket (64 Byte in full-speed mode, 512 Byte in high-speed mode)
 *  - PIO mode
 *  - AHB_DMA mode (??)
 *
 */
...
static void EPX_check_advance(struct FTC_udc *dev, struct FTC_ep *ep)
{
	struct FTC_request *req;
	...
	if (unlikely(list_empty(&ep->queue))) {
		...
		if (dev->EPUseDMA == FUSB220_DMA_IS_IDLE) {
			req = list_entry(ep->queue.next, struct FTC_request, queue);
		...
		if ((u32) req->req.length == 0)
			printk(KERN_INFO "Error: Transfer length in request = 0\n");
...
udc_reinit(struct FTC_udc *dev)
{
	...
	for (i = 0; i < FUSB220_SUPPORT_EP; i++) {
...
udc_reset(struct FTC_udc *dev)
{
	...
	/* 
	 * The UNPLUG bit clear should be cleared when any class driver loaded.
	 * If no class driver loaded before USB line is connect, the USB host will start enumeration right
	 * behind load udc driver. Then USB enumeration will fail, an unplug / plug sequence can fix this
	 * bug in Windows PC host.
	 */
...
u8StandardCommand(struct FTC_udc *dev, struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		// reset dma status
		dev->EPUseDMA = FUSB220_DMA_IS_IDLE;	//reset
...
static int FTC_usb_probe(void)
{
	...
	the_controller->EPUseDMA = FUSB220_DMA_IS_IDLE;	//reset
	...
	//<4>.Init USB DEV ISR  
	if (request_irq(IRQ_USBDEV, FTC_irq, IRQF_SHARED,
